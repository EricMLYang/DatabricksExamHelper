## Databricks 效能監控、調整與優化指南（認證考生簡易瀏覽版）
...
2. **結果集太大（maxResultSize 撐爆）**

* 就算不是 collect，某些 action 回傳資料量過大也可能打到 `spark.driver.maxResultSize`，嚴重時導致 Driver 不穩或 OOM

3. **Broadcast 太大（小表其實不小）**

* Broadcast Join 會把「被廣播那張表」送到每個 Executor，但**廣播的準備/序列化與中繼資訊**也會讓 Driver 壓力暴增
* 典型錯誤：把幾 GB 的表硬當小表廣播

4. **Task/Stage 數量爆炸（排程與 metadata 壓垮 Driver）**

* 分區數極端大、或小檔案多到產生海量 tasks
* Driver 需要持有與排程大量 task 的資訊，容易記憶體吃滿

5. **Query Plan / Lineage 太複雜**
...
如果你希望我把這份「簡易瀏覽版」再壓縮成 **1 頁 A4 考前小抄**（更像考試筆記），我也可以直接幫你重排成更好背的版本。
